...
compatibility_fpm() = (f = zeros(Int, 64); feastinit!(f); f)

@testset "Parallel RCI and banded compatibility" begin
    @testset "Parallel RCI preserves caller seed" begin
        f = compatibility_fpm(); feastdefault!(f)
    ...
    @testset "Banded custom contour budget" begin
        cf = compatibility_fpm(); cf[8]=256; cf[16]=1
        c = feast_gcontour(1.5+0im,0.75,cf)
        f = compatibility_fpm(); f[8]=8; f[4]=1; f[16]=1
...
@testset "Parameter wrappers and contour budgets" begin
    @testset "Parallel saturation agrees with serial" begin
        for storage in (Matrix, sparse), m in (1, 3)
            ...
            r = solver(A, copy(A), 0.5, 1.5, m, compatibility_fpm())
    ...
    @testset "Dense general job budget follows the actual contour" begin
        ...
        for custom in (false,true)
            f = compatibility_fpm(); f[2]=1; f[8]=32; f[4]=1; f[16]=1
            if custom
                # More nodes than either of the normal-contour settings.
                cf = compatibility_fpm(); cf[8]=128; cf[16]=1
                c = feast_gcontour(1.5+0im,0.75,cf)
...
@testset "Storage precision and API compatibility" begin
    @testset "Single precision sparse solves" begin
        for CT in (Float32, ComplexF32), generalized in (false, true), cache in (0, 1)
            ...
            f = compatibility_fpm(); f[10] = cache
            ...
            # Float32 convergence has a sqrt(eps) floor. The solver scales
            # residuals by max(|λ|, σ), where σ ≤ ‖A‖₂ is estimated from
            # random probes, so the accepted ‖Ax - λx‖ varies with the
            # probes. The normwise backward error below is bounded by the
            # solver residual for any such σ, so it holds on every run.
            tol = FeastKit.feast_tolerance(f, Float32)
            @test all(<=(tol), r.res)
            normA = opnorm(Matrix(A)); normB = opnorm(Matrix(B))
            backward_error = maximum(
                norm(A*r.q[:,j] - r.lambda[j]*(B*r.q[:,j])) /
                ((normA + abs(r.lambda[j])*normB) * norm(r.q[:,j])) for j in 1:r.M)
            @test backward_error <= 1.01f0 * tol
        end
    ...
    @testset "Structured count estimator inputs" begin
        A = Diagonal([1.,2.,3.]); f=compatibility_fpm(); f[2]=32
