...
Weights already include `dz/(2π*im)`. Solve with `feast(A, contour)` or
`feast(A, B, contour)`. No parameters are modified here.
...
Use with `feast(A, contour)` or `feast(A, B, contour)`.
These full contours are not the half-contours returned
...
boundary. Solve with `feast(A, contour)` or `feast(A, B, contour)`; the rectangle
selects eigenvalues, including close to its corners.
"""
function feast_rectangle(xmin::Real, xmax::Real, ymin::Real, ymax::Real;
    ...
    return FeastContour{T}(nodes, weights, corners)
end
