...
- **Parallel:** High-level threaded and distributed backends support sparse real symmetric standard/generalized problems. MPI supports real symmetric standard/generalized problems and dense/sparse complex Hermitian/general problems through `mpi_feast`, `mpi_feast_general`, and `backend=:mpi` with an explicit communicator. Real symmetric PFEAST-compatible aliases (`psfeast_*`, `pdfeast_*`) cover dense/sparse standard and generalized paths plus the parallel RCI entry point; complex MPI aliases (`pcfeast_*`, `pzfeast_*`, `pcifeast_*`, `pzifeast_*`) cover direct and GMRES-backed dense/sparse Hermitian/general paths.
...
2. **MPI/Parallel Families**
   - High-level `backend=:mpi` and `mpi_feast` cover the real symmetric FEAST path and dense/sparse complex Hermitian path.
   - High-level `backend=:mpi` and `mpi_feast_general` cover dense/sparse complex general FEAST.
   ...
   - Complex precision-prefixed PFEAST aliases (`pcfeast_heev/hegv/geev/gegv/hcsrev/hcsrgv/gcsrev/gcsrgv`, `pzfeast_*`) and GMRES-backed aliases (`pcifeast_*`, `pzifeast_*`) route to serial solvers without `comm=` and to MPI kernels when `comm=` is provided.
   - Remaining parity work is threaded/distributed complex/general high-level backends and broader PFEAST example ports.
...
2. Extend non-MPI threaded/distributed complex/general backend coverage if needed by downstream workloads.
