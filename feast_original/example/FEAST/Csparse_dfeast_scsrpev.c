...
// A2
  ...
  *(isa+2*(N+1))=1;
  ...
  fpm[17]=7; //! ellipse 7%
  d=2;
  dfeast_scsrpev(&UPLO,&d,&N,sa,isa,jsa,fpm,&epsout,&loop,Emid,&r,&M0,E,X,&M,res,&info);

  /*!!!!!!!!!! REPORT !!!!!!!!!*/
  printf("FEAST OUTPUT INFO %d\n",info);
  if (info!=0)  printf(" Csparse_dfeast_scsrpev  -- failed\n");
  if (info==0) {
    printf(" Csparse_dfeast_scsrpev  -- success\n");
    printf("*************************************************\n");
    printf("************** REPORT ***************************\n");
    printf("*************************************************\n");
